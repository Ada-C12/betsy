...
describe Order do
  ...
  describe "Custom methods" do
    ...
    describe "wizard_items_total" do
      it "accurately calculates the total" do
        expect(order1.wizard_items_total(wizard1).amount).must_equal 3000 * 10**-2
      end
      
      it "only includes order_items from that wizard in calculation" do
        expect(order1.wizard_items_total(wizard1).amount).must_equal 3000 * 10**-2
      end
    end
    ...
    describe "complete_order" do
      ...
      let (:order_items3) {
        order_items(:order_items3)
      }

      it "can change order to complete" do
        expect(order2.status).must_equal "paid"

        order_items2.shipped = true
        order_items2.save
        order_items3.shipped = true
        order_items3.save
        
        order2.complete_order
        
        expect(order2.status).must_equal "complete"
      end
      
      it "does not change order to complete if even one item in order is not yet shipped" do
        order_items3.shipped = true
        order_items3.save
