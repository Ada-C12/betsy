  ...
  describe 'custom methods' do
    describe 'products_by_category' do
      it 'returns products for a give cateogry given the string name' do
        ...
        Product.create(name:"Lemon Thing", price: 25, stock: 5, img_url:"https://img.com", user: users(:ada), description: "description", category_ids: [category.id])
        lemon_products = Category.products_by_category("lemon")
        expect(lemon_products.count).must_equal 1

        Product.create(name:"Lemon Thing2", price: 25, stock: 5, img_url:"https://img.com", user: users(:ada), description: "description", category_ids: [category.id])
        lemon_products = Category.products_by_category("lemon")
